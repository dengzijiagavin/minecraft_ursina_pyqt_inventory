...
ipconfig=enterbox('你的IP要脸那？布吉岛用一个空格')
if ipconfig == ' ':
    ipconfig = 11451
else :
    ipconfig=int(ipconfig)
msgbox('bind在%s端口'%ipconfig)
...
while 1:
    try:
        connection = socket.create_connection(('localhost', ipconfig))
    except :
        ipconfig = enterbox('你的IP要脸那？布吉岛用一个空格')
        if ipconfig == ' ':
            ipconfig = 11451
        ...
        msgbox('ohhhhhh trueeeeeeee I will sleep again ohhhhhh')
        break
